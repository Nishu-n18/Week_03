...
public class StringConcatenationPerformance {
    ...
    // Using String, it takes O(N^2) time complexity
    ...
    // Using StringBuilder, it takes O(N) time complexity
    ...
    // Using StringBuffer,it takes O(N) time complexity but also a thread safe
    ...
    public static void compareConcatenationPerformance() {
        System.out.println("Comparing String Concatenation Performance");

        for (int N : TEST_SIZES) {
            if (N <= 10000) { // Avoid long runtime for large N using String
                long timeString = testStringConcatenation(N) / 1_000_000;
                System.out.printf("N: %,d | String: %,d ms%n", N, timeString);
            } else {
                System.out.printf("N: %,d | String: Unfeasible%n", N);
            }
            ...
            System.out.printf("N: %,d | StringBuilder: %,d ms | StringBuffer: %,d ms%n",
